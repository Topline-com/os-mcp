...
const STATIC_ENTRIES: CatalogEntry[] = [
  ...
  { name: "spend_transactions", category: "Revenue", description: "Spend transactions across pluggable providers (Brex, QuickBooks Online, ...). Live-fetched per tool call against configured providers; D1 persistence is a future enhancement. Exposed via topline_list_spend_transactions / topline_get_channel_spend / topline_reconcile_spend. Channel classification driven by marketing_config.spend_rules.", status: "exposed", notes: "No CRM endpoint — sourced from external accounting/expense APIs gated by env credentials. Providers self-detect: TOPLINE_BREX_API_KEY for brex, TOPLINE_QBO_REFRESH_TOKEN+CLIENT_ID+CLIENT_SECRET+REALM_ID for qbo." },
  ...
  { name: "ad_publishing_google", category: "Marketing", description: "Google Ads: integration, ad accounts, campaigns, audiences, conversions, targeting search, reporting. Tools (topline_google_* umbrella) are shipped; PIT-level reachability depends on whether the Google ad-publishing scope is present on the regenerated PIT.", status: "requires_oauth", endpoint: "/ad-publishing/google/", notes: "Probed 2026-05-23: GET /ad-publishing/google/integration returned 401 'token invalid' under a Select-All PIT that worked for /ad-publishing/facebook/integration. Tested again 2026-05-23 after connecting Google Ads in the CRM's Integrations UI — same 401, so the cause is NOT a missing integration connection. Most likely the Google ad-publishing scope isn't included in PIT 'Select All' (or wasn't at the time the PIT was generated). Try regenerating the PIT — if 401 persists, the scope likely requires a marketplace OAuth app." },
  { name: "ad_publishing_linkedin", category: "Marketing", description: "LinkedIn Ads: integration, ad accounts, campaign groups, lead forms, targeting search, reporting. Tools (topline_linkedin_* umbrella) are shipped; PIT-level reachability depends on whether the LinkedIn ad-publishing scope is present on the regenerated PIT.", status: "requires_oauth", endpoint: "/ad-publishing/linkedin/", notes: "Probed 2026-05-23: same diagnosis as ad_publishing_google — 401 under Select-All PIT, persists after connecting LinkedIn Ads in the CRM's Integrations UI. Try PIT regen first; marketplace OAuth as fallback." },
