...
- `remote.ts` accepts provider tokens on the existing `/query/api/*` surface only after `unwrapToken` proves expiry, the exact `/mcp` audience, the `mcp` token and grant scopes, OAuth client binding, user-to-connection binding, and a live encrypted connection record. The provider's normal protected-route handler performs the equivalent checks for `/mcp`.
...
The `v2` Wrangler migration introduces the `OAuthFlowDO` class (`v1` already introduced `LocationDO`). Cloudflare cannot roll a deployment back across that Durable Object lifecycle change. After `v2` reaches production, do not use `wrangler rollback` to select `v1` or any earlier version. Prepare rollback as a forward deployment that restores the previous request behavior while retaining the `OAuthFlowDO` export, binding, and complete migration history. Leave both KV namespaces intact; if storage configuration itself must be restored, rebind only to the previous dedicated OAuth namespace. Never point `OAUTH_KV` at `CONNECTIONS`. Provider-issued sessions may require reauthorization if their dedicated namespace is unavailable, while cid, legacy signed, and raw-PIT compatibility paths continue to use the unchanged encrypted connection directory.
